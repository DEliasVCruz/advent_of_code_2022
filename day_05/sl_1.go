...
func main() {
	...
	for scanner.Scan() {
		...
		for i := 0; i < instructions[0]; i++ {
			...
			lastIndex := len(stacks[fromStackIndex]) - 1
			crate := stacks[fromStackIndex][lastIndex]

			stacks[fromStackIndex] = stacks[fromStackIndex][:lastIndex]
			stacks[toStackIndex] = append(stacks[toStackIndex], crate)
		}
